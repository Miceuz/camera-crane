...
#define CMD_CALIBRATE  99 // stop all (hard)
...
void loop() {
  ...
  while (true)
  {
    ...
    if(joystickDeadman.fell()) {
      delay(100);
      TCCR1B = 0;
      joystickSetup();
      while(LOW == joystickDeadman.read()) {
        processJoystick();
        joystickDeadman.update();
      } 
      delay(500);
...
byte processUserMessage(char data)
{
  ...
  case MSG_STATE_CMD:
    ...
    else if (lastUserData == 'c' && data == 'l')
    {
      userCmd.command = CMD_CALIBRATE;
      msgState = MSG_STATE_DONE;
    }
